...
plot(X$AAPL,type = "p", col = "red")
plot(exp(cumsum(X$AAPL)))
...
png(file=paste("fitFIN1.png",sep = ""),width     = 10,
    height    = 10,
    units     = "in",
    res       = 500)
plot(prc, col = 4, pch = 16,ylim = c(-500,900),xlab = "",ylab = "AAPL price and log returns",xaxt = "n",yaxt = "n")
axis(1, at=seq(1,length(X$AAPL), by = 50), labels=X$date[seq(1,length(X$date), by = 50)],las=2)
...
lines(rep(240,length(X$AAPL)))
points(1700*X$AAPL,col=2, pch = 1)
lines(rep(-240,length(X$AAPL)))
...
lines(115*(vstates)-500,lwd=2,col = "yellow")
...
png(file=paste("testFIN1.png",sep = ""),width     = 10,
    height    = 10,
    units     = "in",
    res       = 500)
plot(prc+700, col = 4, pch = 16,ylim = c(-100,900),xlab = "",ylab = "AAPL price and log returns",xaxt = "n",yaxt = "n")
points(prc.ar+700,pch = 2,col=5)
points(prc.lasso+700,pch = 3,col=17)
points(prc.ridge+700,pch = 4,col=14)
points(prc.cimpa+700,pch = 6,col=8)
points(prc.proph+700,pch = 7,col="pink")
points(prc.vanil+700,pch = 8,col ="yellow")
points(prc.pred+700,pch = 5,col="green")
axis(1, at=seq(1,length(X.test$AAPL), by = 50), labels=X.test$date[seq(1,length(X.test$date), by = 50)],las=2)
axis(2, at=c(seq(round(min(prc))+780,round(max(prc)+785),50)), labels = c(seq(round(min(prc))+85,round(max(prc))+85,50)))
axis(2, at=c(-100,15,130), labels = c(0,1,2), las = 1)
axis(2, at=c(290,500,710), labels = c(-0.12,0,0.12), las = 2)
lines(rep(760,length(X.test$AAPL)))
...
points(1700*X.test$AAPL+500,col=2, pch = 1)
lines(rep(240,length(X.test$AAPL)))
#lines(rep(2.6,length(data.example2$total_bases)),col=3,lwd=4,lty = 1)
#points(testtot,col=3, pch = 17)
#points(testmeth,col=7, pch = 17)
#lines(10*res1[1,] - 11,col=8)
#lines(230*(posts$S1)-500,lwd=3,col = "sienna4")
lines(115*((vstates.pred-1))-100,lwd=2,col = "yellow")
lines(115*((states-1))-100,lwd=2,col = "green")
#lines(10*g(fm5$summary.random[[2]]$mode)-13,lwd=4,col = 1)
#lines(10*g(fm4$summary.random[[1]]$mean)-13,col =6,lwd=2)
dev.off()


save.image("sappledf.RData")
