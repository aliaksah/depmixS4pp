...
cors = cor(X[,-1])
slectid = c(1,which(abs(cors[1,])>0.34)+1)
...
png(file=paste("fitFIN.png",sep = ""),width     = 10,
    ...
    units     = "in",
    res       = 500)
plot(prc, col = 4, pch = 16,ylim = c(-500,900),xlab = "",ylab = "AMZN price and log returns",xaxt = "n",yaxt = "n")
axis(1, at=seq(1,length(X$AMZN), by = 50), labels=X$date[seq(1,length(X$date), by = 50)],las=2)
...
lines(rep(240,length(X$AMZN)))
points(1700*X$AMZN,col=2, pch = 1)
lines(rep(-240,length(X$AMZN)))
...
prc.ar = 753.67*exp(cumsum(pred.ar))
