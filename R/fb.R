...
fb <- function(init,A,B,ntimes=NULL,return.all=FALSE,homogeneous=TRUE,useC=F,na.allow=TRUE) {
	...
	#print(gamma)
	gamma[is.nan(gamma)] = mean(gamma,na.rm = T)
	gamma[is.na(gamma)] = mean(gamma,na.rm = T)
