    ...
    for(case in 1:lt) {
      ...
      if(ntimes[case]>1) {
        for(tt in ((bt[case]+1):et[case])) {
          for(j in 1:ns) {
            if(!homogeneous) {
              delta[tt,j] <- max(delta[tt-1,]*(A[tt,j,]),na.rm = T)*B[tt,j]
              if(is.infinite(delta[tt,j]))
                delta[tt,j] = 1
              k <- which.max(delta[tt-1,]*A[tt,j,])
            } else {
              delta[tt,j] <- max(delta[tt-1,]*(A[1,j,]),na.rm = T)*B[tt,j]
              if(is.infinite(delta[tt,j]))
                delta[tt,j] = 1
              k <- which.max(delta[tt-1,]*A[1,j,])
            }
            if(length(k) == 0 || is.infinite(k)) k <- 0 # what's this doing here??? can this ever occur? FIX ME
            psi[tt,j] <- k
          }
      ...
      if((length(et[case])==length(which.max(delta[et[case],]))) & length(et[case])>0)
        state[et[case]] <- which.max(delta[et[case],])
      # this doesn't need a for loop does it???? FIX ME
      if(ntimes[case]>1) {
        for(i in (et[case]-1):bt[case]) {
          if((length(state[i]) == length(psi[i+1,state[i+1]])) &(length(psi[i+1,state[i+1]]))>0)
...
em.mix <- function(object,maxit=100,tol=1e-8,crit=c("relative","absolute"),random.start=TRUE,verbose=FALSE,classification=c("soft","hard"),na.allow=TRUE,...) {
	...
	if(random.start) {
      ...
      fbo$gamma[is.na(fbo$gamma)] = mean(fbo$gamma,na.rm = T)
